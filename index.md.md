# 双 A100 单机多卡 vLLM 部署调优全记录：性能对比与踩坑总结

> 模型：Qwen2.5-7B-Instruct · GPU：A100 80GB PCIe ×2 · 推理框架：vLLM 0.6.10

## 一、摘要

在双 A100 80GB PCIe 服务器上部署 Qwen2.5-7B-Instruct 的过程中，我对单卡/双卡、纯 FP16/FP8 KV Cache 四种组合做了一轮系统性的压测，覆盖了短文本低并发和长文本高并发两个典型场景。结果比我预想的有意思得多——**短文本低并发时，双卡张量并行（TP2）的吞吐反而不如单卡；长文本高并发下，性能直接翻倍**。双卡一定比单卡快吗？做完这轮测试后，我对这个问题的回答是：看情况。
...
![记录空闲显存基线](单卡 FP16（--tensor-parallel-size 1）/记录空闲显存（基线）.png)
...
![运行短文本低并发测试](单卡 FP16（--tensor-parallel-size 1）/运行短文本低并发测试.png)
...
![短文本各项数据](单卡 FP16（--tensor-parallel-size 1）/短文本各项数据.png)
...
![长文本各项数据](双卡 TP2 FP16/长文本各项数据.png)
...
![FP8 KV Cache 短文本数据](单卡 FP16 + FP8 KV Cache/短文本各项数据.png)
...
![启动双卡 TP2 FP16 服务](双卡 TP2 FP16/双卡 TP2 FP16启动服务.png)
...
![短文本显存峰值](单卡 FP16（--tensor-parallel-size 1）/短文本显存峰值.png)
