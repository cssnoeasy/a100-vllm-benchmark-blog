...
| 第 5 周服务化记录 | 脱敏后的服务化边界、能力矩阵和 runner 工程证据，不迁入私有部署手册 | B/C：runner 与就绪检查已验证；网关能力保持有限验证或待验证 |
| 补充任务量化 | A100 量化容量与质量报告、图表 | A/B：实验报告，需保留限定条件 |
| 已完成实验的跨文档汇总 | 部署优化结果总览，将 TP1/TP2、量化、SLO 容量和质量结果按部署决策整理 | A/B：仅汇总现有报告，不新增实验或派生数据 |
| 第 3 周旧博客与补充实验 | 将旧博客中的实验动机、`gpu-memory-utilization`、TP/PP、NCCL/NUMA、KV Cache 检查整理为主文章和补充实验记录 | B/C：来源为已完成记录；按样本量保留边界，不混入正式基线 |
...
第 3 周旧博客原始仓库位于暑期任务目录下的 `第3周：A100受控实验——单卡vs TP=2/blog-source/`，本次整理只读取并提炼其中已经完成的博客和补充实验材料，原始 `blog-source` 未修改。旧博客中 vLLM 0.6.10 阶段的探索数据不与 vLLM 0.19.0 正式基线混写；可公开结论以主项目内新整理的报告为准。
